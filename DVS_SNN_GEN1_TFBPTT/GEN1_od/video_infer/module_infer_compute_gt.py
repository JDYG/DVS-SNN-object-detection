...
class Gen1DetectionModule(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # events_list 是长度为50的list, 每个list里面还是一个list（ 每个list里面包含batch size个Tensor，每个tensor是Nx4，N是events的数目)
        # label是size 为batch size大小的list, 每个list里面是一个dict{'times':, 'bboxes':, 'labels':, 'ignore_mask':}
        ...
        for time_idx, events in enumerate(events_list):
            ...
            # print(self.model.latent_mem.latent_memory.tau)
            ...
            if time_idx >= warm_up -1 and time_idx in gt_index:
                ...
                # preds = postprocess(preds, self.nc, conf_thre=0.3, nms_thre=0.5, class_agnostic=False)
...
# the output targets:[time_index, class, n_cx, n_cy, n_w, n_h]
