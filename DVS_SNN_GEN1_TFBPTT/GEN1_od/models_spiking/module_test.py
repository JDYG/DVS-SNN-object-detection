...
from utils.common import parse_label_data_yolo_seg, parse_event_data, postprocess, parse_label_data_yolo
...
from utils.common import split_list
...
class Gen1DetectionModule(L.LightningModule):
    def __init__(self, cfg):
        ...
        self.cfg_segment = cfg['cfg_segment']
    ...
    def training_step(self, batch, batch_idx):
        ...
        seg_num = self.cfg_segment['seg_num']
        move_per_seg = self.cfg_segment['move_per_seg']
        move_steps = self.cfg_segment['move_steps']
        seg_events = split_list(events_list, seg_num)
        seg_targets, seg_valid_mask = parse_label_data_yolo_seg(label, seg_num, move_per_seg)


        for seg_idx, (events_list, targets, valid_mask) in enumerate(zip(seg_events, seg_targets, seg_valid_mask)):
            opt.zero_grad()
            outputs = self.model(events_list, seg_idx, move_steps)
            loss, loss_items = self.compute_loss(outputs, targets, valid_mask)
            self.manual_backward(loss)
            self.clip_gradients(opt, gradient_clip_val=100, gradient_clip_algorithm="norm")
            opt.step()
            self.model.detach()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for seg_idx, (events_list, targets, valid_mask) in enumerate(zip(seg_events, seg_targets, seg_valid_mask)):
            preds, outputs = self.model(events_list, seg_idx, move_steps)

        targets = seg_targets[-1]
        valid_mask = seg_valid_mask[-1]
        loss, loss_items = self.compute_loss(outputs, targets, valid_mask)
        functional.reset_net(self.model)
        ...
        if plot:
            ...
            for indx in range(targets.shape[0]):
