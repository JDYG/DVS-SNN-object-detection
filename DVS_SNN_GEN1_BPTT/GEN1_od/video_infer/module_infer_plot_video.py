...
compute the preds at every time index for plot video outputs
if you just want to compute the preds for calculatin the mAp at gt, use the module_infer_compute_gt.py
'''
...
class Gen1DetectionModule(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for time_idx, events in enumerate(events_list):
            ...
            preds, outputs = self.model(events, time_idx) # pred (cx,cy, w,h, obj_conf, class...)
            if time_idx >= warm_up -1:
                bs, img_h, img_w = len(events_list[0]), 240, 304
                # after postprocess, yolox: (xl, yl, xr, yr, obj_conf, class_conf, class)
                preds = postprocess(preds, self.nc, conf_thre=0.8, nms_thre=0.5, class_agnostic=False)
