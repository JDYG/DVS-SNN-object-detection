...
def count_spikingjelly_conv2d(m, x, y):
    ...
    total_ops = y.numel() * kernel_ops
    m.total_ops += torch.tensor([total_ops], dtype=torch.float64, device=m.total_ops.device)
...
class repn(nn.Module):
    ...
    def forward(self, events_list):
        for time_idx, events in enumerate(events_list):
            ...
            # events 里面是batch size个list，每个list里面是一个tensor(Nx4)，N是events的数目, p:-1/1
